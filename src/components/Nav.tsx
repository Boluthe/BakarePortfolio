...
export default function Nav() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-slate-200 dark:border-white/[0.1] light:border-slate-200 bg-white/85 dark:bg-[#161a22]/85 light:bg-white/95 backdrop-blur-md shadow-sm dark:shadow-lg dark:shadow-black/20 light:shadow-slate-200 transition-colors duration-300">
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-6 py-3.5">
        ...
        >
          <span className="h-2.5 w-2.5 rounded-full animate-color-cycle transition-transform group-hover:scale-125 shrink-0" />
        ...
        {/* Mobile top bar actions (`md:hidden`) */}
        <div className="md:hidden flex items-center gap-2">
          {/* Theme Toggle Icon for Mobile (Outside Hamburger) */}
          <button
            onClick={toggleTheme}
            aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
          ...
          >
            {theme === "dark" ? (
              ...
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="text-sky-500">
                <path strokeLinecap="round" strokeLinejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
              </svg>
            )}
          </button>
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200 dark:border-white/[0.1] bg-white/95 dark:bg-[#161a22]/95 backdrop-blur-xl transition-all duration-300">
